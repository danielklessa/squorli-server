...
The four known gaps of the plan cleanup are closed (25 September 2026: signed attachment links, `VIEW_VIDEO` per channel, radio playlists against DNS rebinding, the desktop app's keys in `safeStorage`; each in its feature note). The systematic pass ran on 25 September 2026 (four reviewers: sign-in and sessions, server accounts, chat server and directory, browser surface and files). The user's decision: fix everything, in four groups. Group A is done (the login relay through a foreign server's own domain, SVG attachments on the server's origin, the directory's host proof: each in its feature note). Open, in the order they will be fixed:
- **Group B:** the directory lets any registered server add itself to any account's server list and clients connect to those on their own (fix: the directory records a membership only with the user's signed proof, and the client connects on its own only to servers it knows); the claim of a member from before uploads the main identity's key (fix: a fresh key per server and a move of the membership signed by the old key); the server accounts' auth key is derived like the directory's (fix: bind the derivation to the server's domain, old backups stay readable and switch at the next sign-in).
