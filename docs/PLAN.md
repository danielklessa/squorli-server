...
Committed with server 0.1.1 the same day: `squorli restore`, the LiveKit pin, the plan cleanup (website and directory with it). After that, together in the next server release **and** a desktop app release:
- rate limits on every public path (`docs/features/rate-limits.md`, new optional variable `RATE_LIMIT_FACTOR`; release notes: operators behind a proxy on another host must have its address in `TRUSTED_PROXIES`, otherwise all clients share one address and hit the IP limits together);
...
- security review groups A and B: the sign-in signs the connected host (client), safe attachment types (server), the membership proof passed to the directory (server; a server of an older version adds no new server-list entries once the directory is updated), the claim on a fresh key (server and client together: a current client claims only on a server with `localClaimRekey`), backups bound to the server (client). Release notes: operators should update the server and the directory; nothing breaks either way round.
...
The four known gaps of the plan cleanup are closed (25 September 2026: signed attachment links, `VIEW_VIDEO` per channel, radio playlists against DNS rebinding, the desktop app's keys in `safeStorage`; each in its feature note). The systematic pass ran on 25 September 2026 (four reviewers: sign-in and sessions, server accounts, chat server and directory, browser surface and files). The user's decision: fix everything, in four groups. Groups A and B are done (the login relay through a foreign server's own domain, SVG attachments on the server's origin, the directory's host proof; the server list only with the user's proof and no connection by itself to unknown servers, the claim on a fresh key, backups bound to the server: each in its feature note). Open, in the order they will be fixed:
- **Group C:** parallel requests pass the password failure limit (`auth/local.ts`); rate limits per exact IPv6 address instead of the /64, no per-account limit on the password change and deletion; `/api/local/claim` without a signature or a limit; two first sign-ins at once both become owner; the web client without a CSP, `frame-ancestors` or `Referrer-Policy`; `javascript:` links through link previews, foreign attachments and a foreign `directoryUrl`.
- **Group D:** signing out the current session leaves its sockets open; sockets are never checked again after `hello` (expiry, a kick in between), a second `hello` is taken; kicked or banned members keep their sessions (`PATCH /api/me`, the avatar routes); `squorli://control/mic-on` from a web page; the status key and the attachment signature in request logs; the client's directory signatures bound to the host the directory names; `/api/directory/leave` tells the owner's key; the server does not check that the directory's answer is for the key it asked about; forged notify pushes can keep a failed registration failing; session tokens stored in plain text; handles can be enumerated; a password change keeps other sessions. A browser page cannot encrypt its key (no key store a page can reach); that stays a documented limit (`docs/features/desktop.md`).
