...
/**
 * A video's title, asked from YouTube's public oEmbed endpoint when a member starts the video (a fixed address of
 * youtube.com, nothing the member typed is fetched). It also tells a video that does not exist or may not be embedded,
 * which would otherwise only show as an error inside everyone's player. YouTube not reachable from here is no reason to
 * refuse: the clients' players talk to YouTube themselves, the radio then simply has no title.
 */
