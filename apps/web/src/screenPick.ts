 ...
 * have any: `audio` false), a screen's does not; a detected game is sent with H.264, everything else with the standing codec.
 */
export const defaultAudio = (source: ScreenSource | null): boolean => source?.kind === "window" && source.audio;
export const defaultCodec = (source: ScreenSource | null): ScreenCodec => (source?.kind === "window" && source.gameId ? "h264" : "vp8");
...
/** "Quick Share": the running game's window, topmost first, with its audio and always H.264, without the dialog; null = that window is not on offer (minimized, an older shell), then the dialog opens. */
export function quickSharePick(sources: readonly ScreenSource[], gameId: string): ScreenPick | null {
  const source = sources.find((s) => s.kind === "window" && s.gameId === gameId);
  return source ? { sourceId: source.id, audio: source.audio, codec: "h264" } : null;
}
