...
/** One line: the game's icon (a gamepad where it has none) and the game's name, without a "Spielt" before it (user's wish, 21 September 2026: the icon says it). Where there is nothing to show, `fallback` (or nothing). */
export function GameLine({ game, className = "", fallback = null }: { game: GamePresence | null; className?: string; fallback?: ReactNode }) {
  ...
  return (
    <small className={`game-line ${className}`} title={shown.name}>
