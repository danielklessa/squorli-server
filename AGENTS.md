...
| Status API (`GET /api/status`: name, icon, channels in order, members with their voice channel and mute state; off by default, with the server's key or public, Verwaltung > Server) and the mute/sound-off display for the whole server (`voice.join`/`voice.status`, `VoiceMember.micMuted`/`deafened`) | `docs/features/status-api.md` |
