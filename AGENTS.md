...
| `pnpm smoke` | Smoke test (auth, owner, roles "Gast"/"Mitglied", invites, sessions/devices with remote logout, structure, permissions, messages, attachments, WS, RTC token grants, voice channel moderation, kick/ban, voice profile per channel, appoint/revoke owner, server icon, only-with-account, server directory listing, directory handle and display names and account deletion through the directory if `DIRECTORY_URL` is set; 190 checks without a directory, counted on 24 September 2026 with the channel blocks (20 of them the section "Channel permissions": private channels, the write rules, slowmode, user limit, sticky, moves, eviction; 14 of them the section "Vote kick"; 8 of them the section "Channel blocks") (159 when server and script run with `LINK_PREVIEW_TEST_ORIGIN=http://127.0.0.1:3198`: the script then plays a linked website and the server may fetch from that one local address), the game display, read states, mutes, the mention rules, the web radio (4 of them the YouTube playlist queue, which like the YouTube video check ask YouTube's oEmbed), the AFK detection and `inviteRequired` in `/api/health` (one of them, the idle stop's timing, only when server and script run with `RADIO_IDLE_STOP_MS=2000`); refuses to run when the server names a directory that is not on localhost; the run with a directory adds its own: 129 checks on 19 September 2026, 5 of them the avatar of the directory account), **needs a running server on :3000** (`SMOKE_URL` for others). Remembers the owner key in `apps/server/scripts/.smoke-owner.json`. **Best against a separate test DB** (see pitfalls), because the first login becomes the owner. |
